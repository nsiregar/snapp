...
mod CounterContract {
  ...
  impl CounterImpl of ICounter<ContractState> {
    ...
    fn increase_counter(ref self: ContractState) {
      ...
      let new_counter_value = current_counter + 1;
      self.counter.write(new_counter_value);

      let event: CounterChanged = CounterChanged {
        old_value: current_counter,
        new_value: new_counter_value,
    ...
    fn reset_counter(ref self: ContractState) {
      ...
      assert!(balance >= payment_amount, "User don't have enough balance");
